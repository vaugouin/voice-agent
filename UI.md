...
- Credits, in order: "Design by A Fistful of Reels" (links to the X profile), "Built with AI agents", "Voice via OpenAI Realtime API", and "Picture recognition via OpenAI GPT-6 Astra". The last one is the only credit naming a model rather than an API, so it is also the only one that can quietly become false: the photo is read upstream by `fastapi-text2sql`, whose `llm_model_vision` defaults to `gpt-6-astra` and also accepts `gpt-4o` and the GPT-5.x line. Re-check it against `strvisionmodeldefault` in the API when the vision model changes.
...
Elements: `#lookToggleButton`, `#lookMenu`, `#lookCameraInput`, `#lookLibraryInput`, `#lookDropOverlay`, `#lookPreviewOverlay`, `#lookAttachment`

Purpose: attaches a photo to the conversation so a question can be asked about it. A click opens a source menu, the chosen photo is resized in the browser, shown for confirmation, and deposited on the text2SQL API; the question itself is asked by the next submission, by voice during a live session and through `/text-chat` otherwise. The button is an action, not a toggle: it has no on/off state and never shows a cross.

**Attaching is not sending (VOICE-AGENT-183).** Confirming a photo files the image and hands the screen back; nothing is searched and nothing is answered. The photo is then **armed**, and the next turn the user submits is the question about it: the submit arrow, `Enter`, or the next sentence spoken into a live session. The dropped or chosen bytes do go up to the API at the moment of the attach, because that is the image being filed and not the question being asked.
...
### Dropping a picture on the page (VOICE-AGENT-184)

Element: `#lookDropOverlay`

- A picture dragged from the desktop onto **anywhere on the page** enters at the same door as one chosen from the library: same resize, same confirmation overlay, same attachment. The source is recorded as `drop`; the chip therefore reads `Picture`, and `Retake` on a dropped photo falls back to the library picker, since there is no picker to reopen.
- The drop target is the whole window, not the question box. A one-line strip at the bottom of a screen filled with cards is a precision target, so the veil says the whole page will do.
- The veil appears once when a drag carrying files enters the window and comes down once when it leaves, on the drop, or on `dragend`. It is driven by an enter/leave depth counter, because both events fire for every element crossed; without the counter it would strobe for the length of the journey. It is `pointer-events: none`, so the file lands on the window and never on the veil.
- A drop is refused, and the veil never appears, while the confirmation overlay, a full-size viewer, the burger drawer or the launch splash owns the screen.
- Several files at once attach the **first image** and say so in the subtitle lane, naming the file. Anything that is not an image, and a drag from a web page (which carries a link and no file at all), sets status `Photo error` and gives a plain sentence rather than failing silently. Both outcomes are logged under `look_drop`.
- A file with no MIME type at all is let through on its extension, HEIC being the case that will happen on Windows: the failure then comes from the decoder, with the message the picker already produces, rather than from a blanket refusal.
- **The listeners exist even when the app wants nothing from the drag.** With no listener the browser replaces the page with the dropped file, and this app is a single page whose conversation lives in the tab's memory. `dragover` is prevented on every event, not just the first, or `drop` never fires at all.
- No keyboard shortcut is captured, and the picker and the eye menu are untouched.
...
- `Attach this photo` deposits the image and hands the screen back: the note reads `Attaching the photo…`, status runs `Attaching the photo` and then returns to `Listening` or `Idle`, the overlay closes, the chip appears, and the cursor goes to the question box. **Nothing is searched and nothing is answered.** `Retake` reopens the same source. `Cancel` and `Escape` discard the capture and return the status to its idle text.
- While it is open, the single-key shortcuts are inert (`keyboardOwnedElsewhere()`), so no key acts on the app behind it, and a drop is refused.
...
- The `✕` removes the photo: the chip disappears, the object URL is revoked, and later turns go back to being text-only. Removing an **armed** photo before submitting leaves no trace at all: no turn went out, no screen was repainted, and the submit arrow goes back to following the text box.
...
1. The confirmed bytes are posted to `/tool/vision-upload`, which proxies them to the API's `POST /uploads/vision` and returns an `image_ref`. This is the whole of what the attach does.
2. The photo is now armed. **What spends it**, whichever comes first:
   - the submit arrow or `Enter`, with words or with an empty box (trigger `typed`);
   - the next spoken sentence, during a live session (trigger `spoken`). A voice session has no `Enter` key, so its submission is speech. The turn is taken over by the browser rather than left to the model, which is never told a photo exists and would answer blind; the response the model had already started is cancelled, exactly as a photo has always interrupted a sentence in progress. A sentence said while a photo is armed therefore becomes the question about it, whatever it was about.
3. Which path carries the turn is decided when it is spent, by one test: a live session **with an open data channel** takes the voice path, anything else takes the text path. `sessionRunning` on its own is not enough, because it is also true while connecting, and a turn injected into a channel that is not open yet would be lost without a word.
4. `look_capture` is logged on every outcome (`attached`, `sent`, `cancelled`, `released`). The `attached` entry carries the source, the sizes before and after the resize, the resize and upload times, the reference, and the session state at that moment; the `sent` entry carries `trigger` (`typed` or `spoken`), `mode` (`voice` or `text`) and `session_stopped`. `look_capture_error` covers a capture that never became an attachment, `look_drop` a drop that was refused or accepted.
...
2. It stops the audio transport if one was running, and the subtitle lane says so. This now happens when the turn is **submitted**, not when the photo is attached.
3. Status runs `Preparing the photo` → `Confirm the photo` → `Attaching the photo` → `Listening`/`Idle` (the photo is armed and the app is waiting) → `Reading the photo` → `Text response`.
...
1. **The photo interrupts**, exactly as starting to speak or sending a typed turn does: `response.cancel` plus `output_audio_buffer.clear` if the agent was mid-sentence. It does not queue behind the end of the turn; nothing else in the app queues. On the `spoken` trigger the cancel goes out even when no response has been observed yet, because the server created one at the speech commit and its `response.created` may still be in flight. The response fallback armed by that commit is cleared too, so it cannot create a blind response behind the picture search.
2. The words are the ones **in the question box** on the `typed` trigger, usually empty, and the sentence just spoken on the `spoken` one, which the transcript handler has already announced (subtitle lane, retained context, `lastUserTranscript`) and which is therefore not announced twice. An OLD spoken transcript is never reused in either case: it has already been sent and answered as its own turn.
...
The photo has been spent by then, so these rules apply from the second turn onwards; the first one is governed by the arming rule above.
...
- Content: a `.showcaseViewport` containing a few stacked `.showcaseLane` rows, each a flex scroll container holding `.showcaseGroup` blocks directly (there was an intermediate `.showcaseLaneTrack`; it only existed to carry the marquee's `transform` and was dropped once the auto-scroll moved to the lane's `scrollLeft` — see Hand gestures). Each group is a `.showcaseQuestion` chip (a button) followed inline by standard `.search-poster-card` cards (or a `.showcaseScalar` value for scalar previews). Each lane's groups are rendered twice so the marquee wraps seamlessly. Eligible samples (those whose `simulated_result` has renderable rows) are taken by `selectShowcaseSamples`, round-robined across top-level categories, then distributed across lanes round-robin.

The pool is sized to what the lanes actually show: `laneCount × SHOWCASE_SAMPLES_PER_LANE` (**12** distinct samples per lane, so 24 to 72 in total), **not** a flat 300 as before (VOICE-AGENT-098). The old wall put 100-300 groups in every lane once doubled — a `scrollWidth` of 140k-420k px and 900-2700 elements per lane — which is what made iOS drop a lane's first swipes for seconds while WebKit tiled that scroll layer, then run fine once built; `.detailVisualRail` (~5k px) always swiped instantly on the same device. The cap costs nothing visible: at ~72px/s a lane needs ~4 minutes to loop back to its first group, and a viewer sees 1-2 groups per lane in a 30s glance — the old wall built roughly 100× more DOM than it could ever show. `SHOWCASE_SAMPLES_PER_LANE` is the tuning knob; the `showcase_rendered` log event reports the resulting `scroll_width_max` so the trade-off can be judged on a measurement.
...
- Every `.showcaseLane` is a **native horizontal scroll container** (`overflow-x: auto`) and is itself the flex container holding the groups — deliberately the same shape as `.detailVisualRail`, the one horizontal rail measured to swipe perfectly on iPad. This is why the motion is driven by `scrollLeft` rather than a `transform`: the lane must be something the browser can scroll. A swipe then scrolls the row with the platform's own momentum, and past/future questions can be browsed in both directions.
...
- The viewport's soft left/right edge fade is painted by `.showcaseViewport::before` / `::after` gradient overlays (`pointer-events: none`), **not** by a `mask-image` on the viewport. A mask on the lanes' parent makes WebKit scroll badly inside it: on iPad (Safari *and* Chrome, both WebKit) a lane needed several swipes over several seconds before it moved, while every Windows browser was fine and `.detailVisualRail` — which has no masked ancestor — scrolled fine on the same iPad. The overlays sit on the flat `--bg-secondary` panel, so the fade looks the same.
...
- Supported entity IDs include `ID_MOVIE`, `ID_SERIE`, composite season keys (`ID_SERIE` plus `SEASON_NUMBER` on an `ID_SEASON` row), composite episode keys (`ID_SERIE`, `SEASON_NUMBER`, and `EPISODE_NUMBER` on an `ID_EPISODE` row), `ID_PERSON`, `ID_COMPANY`, `ID_NETWORK`, `ID_T2S_COLLECTION`, `ID_TOPIC`, `ID_T2S_LIST`, `ID_MOVEMENT`, `ID_TECHNICAL`, `ID_GROUP`, `ID_DEATH`, `ID_AWARD`, `ID_NOMINATION`, and `ID_LOCATION`.
- **A location row is named by `LOCATION_NAME`, and that is load-bearing.** While locations were read from `T_WC_T2S_ITEM` their label was `ITEM_LABEL`, which is what the card-spec name chain listed. API 1.1.19 gave them their own table and renamed the column, so the chain stopped naming them: with no name and no image (locations carry neither `POSTER_PATH` nor `WIKIPEDIA_IMAGE_PATH` today), `cardSpecFromRecord()` returned `null` and every place fell through to the aggregate renderer, printing its raw columns including `Id Location`. The lesson generalizes past locations: in that chain, a renamed label column does not fail loudly, it silently downgrades the card.
...
- episode detail pages render parent-series and parent-season navigation, metrics, cast, crew, and returned still-image cards. Metric tiles are Episode #, Aired, Duration, Rating, plus **Votes** (`VOTE_COUNT`, thousands-separated) and **Type** (`EPISODE_TYPE` prettified — shown only for notable types like premiere/finale/mid-season; the common `standard` is omitted) (VOICE-AGENT-091). The episode `OVERVIEW` shows as the tagline. Season detail pages already surface every display-worthy season column (Aired, Episodes, Rating, plus `OVERVIEW` as the tagline); the remaining columns are external IDs (hidden by convention) and internal pipeline timestamps, so no season tiles were added.
...
- movie detail pages (VOICE-AGENT-089) show metric tiles for **Released, Duration, IMDb, Popularity, Budget, Revenue, Original language**, plus **Original title** only when it differs from the shown title, plus **Criterion** (`Spine #1`) only when the film is in the Criterion Collection (VOICE-AGENT-153). The spine tile guards on `Number(ID_CRITERION_SPINE) > 0`, not on emptiness: the column is **0, never NULL**, for a film outside the collection, so a plain empty check would render a tile reading `Spine #0` on every other film. Budget/Revenue use a compact money format (`formatMoney`, e.g. `$15M`) and are dropped when 0; Original language uses `Intl.DisplayNames` (`formatLanguageCode`, `en` → `English`). The **Director tile was removed** — the director already appears in the Crew rail. Below the tiles, three **chip strips** (`appendChipStrip`) render the image-less label lists — **Genres** (from `genres[].GENRE_NAME`), **Spoken languages** (`spoken_languages` codes → names), and **Production countries**. Chip strips are used instead of poster rails precisely because these have no image and image-less rail cards are hidden (VOICE-AGENT-068); an empty list renders nothing.
- **series detail pages mirror the movie tiles/chips** (VOICE-AGENT-090): they add **Popularity, Original language**, and **Original title** (when it differs) tiles next to First aired/Seasons/Episodes/IMDb, drop the **Director tile** (kept in the Crew rail), and render the same **Genres / Spoken languages / Production countries** chip strips above the Seasons rail. Series have no budget/revenue; their networks still render as logo rails via the mixed-relation sections.
- **Countries are shown as a flag emoji** (VOICE-AGENT-090), not the 2-letter code: `countryFlag(code)` returns the Unicode regional-indicator pair (falling back to the raw code when invalid). Applied to a person's **Country** of birth metric tile and to the **Production countries** chips on movie/series pages; the chips keep the full country name (`countryName`) as a hover `title`. `appendChipStrip` accepts either plain strings or `{ text, title }` chips for that tooltip.
- person detail pages (VOICE-AGENT-092) show **Born** and **Died** as full dates (`BIRTHDAY`/`DEATHDAY`, falling back to the year), **Known for**, **Gender** (`formatGender`: 1→Female, 2→Male, 3→Non-binary), **Popularity**, and **Country** of birth (flag), plus an **Also known as** chip strip from `ALSO_KNOWN_AS`/`ALIASES` (capped at 10 via `splitList`).
- the generic detail branch (company, network, collection, topic, list, location, movement, technical, genre, group, death, award, nomination) surfaces whatever scalar fields the entity's own table carries, each tile skip-empty (VOICE-AGENT-092): **Type, Movies, Series, Persons, IMDb, Popularity**, **Country** (`ORIGIN_COUNTRY` flag — company/network), **Headquarters** (company/network), and **Source** (`COLLECTION_SOURCE`/`TOPIC_SOURCE`/`LIST_SOURCE` prettified — the group/death/award/nomination `*_SOURCE` are raw Wikidata property codes and are intentionally omitted). Chip strips: **Applies to** (genre `APPLIES_TO_MOVIE`/`APPLIES_TO_SERIE`) and **Also known as** (`ALIASES`, e.g. locations). The title and `OVERVIEW`/`DESCRIPTION` tagline and poster are already shown by the shared header for every entity.
- **location detail pages split their works by role instead of listing them once.** `LOCATION_ROLE` lives on the association row, not on the place — Paris is the same Paris whether a film was shot there or set there — so one mixed `Movies` rail would drop the only thing that separates "filmed in Paris" from "set in Paris". The page renders four rails instead: **Movies filmed here, Movies set here, TV shows filmed here, TV shows set here**, worded as on the equivalent `tmdb-front` page. `LOCATION_TYPE` (city, country, region, island, structure, nature, fiction) is prettified into the shared **Type** tile. Two consequences to keep in mind when touching the rail code: the four rails are fed by only **two** upstream collections (`movies`, `series`), so a loaded page is partitioned by role on arrival and each half appended to its own rail; and a role rail's header shows **the number of cards it holds**, not `N of M`, because the upstream `total` counts both roles and the API exposes no per-role total. A role rail whose half of the first page came back empty is still drawn when further pages exist, otherwise the rows arriving later would have no rail to land in.
...
| Photo attached, not yet asked about (VOICE-AGENT-183) | unchanged | unchanged | unchanged | enabled | visible/enabled, focused, submit arrow visible | `Attaching the photo`, then back to `Listening`/`Idle` | unchanged, nothing repainted | unchanged | visible | unchanged |
| Photo turn during a session (VOICE-AGENT-180) | hidden | visible/enabled | closed while the photo is read, then follows manual state | enabled | visible/enabled/cleared | visible unless results shown, `Reading the photo`, `Responding`, then `Connected` | visible and cleared, then the recognised entry | hidden if results visible | visible | may show `📷 Photo` in the user lane, or the spoken question, and the answer in the assistant lane |
| Picture dragged over the page (VOICE-AGENT-184) | unchanged | unchanged | unchanged | enabled | unchanged | unchanged | unchanged behind the veil | unchanged | unchanged | unchanged |
