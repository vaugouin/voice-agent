...
- Row cap per group: most samples render up to 8 preview cards. **Image-query samples** — where the API sets `simulated_result.image_gallery` (the movie-posters / series-posters / person-portraits evaluation categories, whose rows are the entity's `*_IMAGE` posters/portraits) — render up to 40, so the group shows the entity's full image set instead of a single card. `buildShowcaseGroup` reads that flag to pick the cap.
