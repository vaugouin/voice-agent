...
- **Agent persona lives in soul files, and is swappable.** Both prompt builders (the Realtime `realtime_session_config()` and the `/text-chat` builder) are prefixed with `soul_instructions(soul)` = **the selected persona, then the non-negotiable core**. Only the *operational* instructions (which tool to call, id-hiding, recovery, disambiguation) stay in code.
  ...
  - A soul file may carry optional front matter with `name` (label shown by `GET /souls` and in logs), `brevity` (`concise` by default, or `expansive` — the one dial a persona may turn: it overrides the per-surface length delta, "Keep spoken answers concise" / "You reply as concise text") and `voice` (the Realtime voice this character speaks with).
  - **One character = one file.** The persona file carries the prose, the `voice`, and — inside its HTML comment, so it is stripped before injection — the *avatar subject description* used to generate `app/static/souls/<slug>.webp`. Prose, voice and face therefore cannot drift apart. The shared image style lock and the generation notes live in `souls/_avatars.md`. Files under `souls/` whose name starts with `_` are never selectable as personas.
  ...
  - **On screen**, the active persona's portrait appears as a circular badge in the bottom-left corner **while the agent speaks** (VOICE-AGENT-121). The browser resolves it once per page load from `GET /souls` and preloads only that portrait. See `UI.md` › *Persona Badge* for the visibility and caption-collision rules.
- **The agent is told today's date, in two places.** Without it the model has no clock and dates every record from its own training: it called an episode broadcast three days earlier "an upcoming air date".
  ...
  - **The comparison is done in code, not by the model.** Telling it to compare dates was measured insufficient: it read a `DAT_RELEASE` of 2024-10-14, had today's date in the same payload, and still answered that the film's release date was later than today, dropping a correct row from its answer. What a model believes about a title beats arithmetic it is asked to perform. So every search row carries `_date_status` (on the row **wrapper**, beside `data`, never inside it, since those dicts are what the browser renders cards from) and every detail response carries `date_status`, each a map of date column to `past` / `today` / `future` computed against the agent's clock. The prompt declares that field authoritative and forbids the model from working the comparison out itself. Columns covered: `DAT_RELEASE`, `DAT_FIRST_AIR`, `DAT_LAST_AIR`, `DAT_AIR`, `DEATHDAY`. An unparseable or absent date yields no verdict, and the prompt tells the model to say nothing about timing when the field is missing.
  - **Both paths carry the verdicts, only one carries the line.** `_with_date_guardrail()` (line + verdicts) runs on the voice tool endpoints; `_with_date_status()` (verdicts only) runs inside `execute_text_tool()`, since the text path already prepends the line to its input. The split exists because the precedence rule *states* that every record carries `_date_status`: shipping that sentence on a path where the field is absent would make the prompt lie about its own payload, and the model would fall through to the "say nothing about timing" clause for no reason.
...
- `app/static/app.js::handleFunctionCall()` is the tool-dispatch loop. It deduplicates `call_id`, parses model arguments, calls the local tool adapter, renders the result in the UI, stores compact tool context, sends `function_call_output` back to Realtime, and asks the model to continue with `response.create`. Because the Realtime API allows only one active response at a time, if a response is already active when the tool output is ready (e.g. a "still loading" filler the model is speaking), the `response.create` is **deferred** (`pendingToolResponseRequest`) and fired when that response ends (`response.done`), instead of being dropped — the dropped-request case previously left `awaitingToolResponse` stuck true, muting the mic and wedging the session (VOICE-AGENT-094). A `scheduleToolResponseWatchdog()` timer is the final backstop: if the model never picks up a sent tool output within a few seconds, it clears the stuck state, un-mutes the mic, and re-triggers the response.
...
Precedence for one session: **`?voice=<name>` on the URL** (the browser forwards it to `/session` alongside `?soul=`) → **the active persona's own `voice:`** (see the soul files above: a character owns its timbre) → `AGENT_VOICE` → the built-in default. So `/?soul=video-store` already speaks with that character's voice, and `/?soul=video-store&voice=cedar` swaps the timbre without touching the character. An unknown or malformed name silently falls back rather than failing the session; an unsupported value in `.env` still raises, because that is a misconfiguration and not a typo in a link.
...
**`reason` also decides whether a turn is allowed to repaint the screen.** When a `query_text2sql` result comes back as a *failure* (`no_sql`, `sql_error`, `entity_unresolved`, `ambiguous`, `transient`, `unknown`) with no rows, the browser **keeps the current screen** instead of clearing it, and logs `forced_search_render_skipped` (`source`, `reason`, `result_count`, `error`). The model still receives the full output and its diagnostic, so grounding and recovery are unchanged: what is withheld is a render, not a fact. `ok` and `empty_result` repaint as before, and the distinction matters, since `empty_result` means the query ran and genuinely matched nothing, which is a true answer to show ("no film matches"), whereas a failure never managed to represent the question at all. The guard is skipped when there was nothing on screen to preserve (cold start), so a first question that fails still produces visible feedback.

This covers **both modes** since VOICE-AGENT-185. It started as a typed-path rule, on the reasoning that `/text-chat` pre-executes a search for every message whereas in voice the model decides to search. What that missed is the follow-up naming no entity: asked out loud "and who are the actors?" with a film on screen, the model sends the words to the search, which is stateless and cannot tell which film is meant, and the film is wiped for the seconds it takes the model to re-route itself to the detail tool. Measured twice in one session on 2026-09-22, on the cast and on the director.

**What the user reads on a failure is never the engine's own words.** The API's `answer` is preferred when it composed one, since it was written for a reader ("Could you please specify the movie or series you are referring to?"); otherwise the `reason` maps to a plain sentence. The raw upstream string stays in the collapsed query-details disclosure and in `logs/client-*.log`.
...
15. Waiting screen (VOICE-AGENT-186): after `New conversation`, type a question and press `Enter`. Until the answer arrives the panel shows the question in large type with `Searching…` under it, then the result replaces it. Attach a photo and press `Enter`: the photo itself fills the panel with `Reading the photo…`. Click the chip's thumbnail while it is being read: the full-size view opened that way must still be on screen when the result arrives underneath.
