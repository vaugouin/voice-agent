...
**`reason` also decides whether a typed turn is allowed to repaint the screen.** On `/text-chat` the server pre-executes one `query_text2sql` for every typed message, and that search is what renders the result grid. When it comes back as a *failure* — `no_sql`, `sql_error`, `entity_unresolved`, `ambiguous`, `transient`, `unknown` — the browser now **keeps the current screen** instead of clearing it, and logs `forced_search_render_skipped` (`reason`, `result_count`, `forced`). The model still receives the full output and its diagnostic, so grounding and recovery are unchanged: what is withheld is a render, not a fact. `ok` and `empty_result` repaint as before, and the distinction matters — `empty_result` means the query ran and genuinely matched nothing, which is a true answer to show ("no film matches"), whereas a failure never managed to represent the question at all. The guard is skipped when the results panel is still hidden (cold start), so a first question that fails still produces visible feedback.
