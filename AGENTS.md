...
- **An image never travels on the WebRTC data channel**, whose maximum message size is the subject of VOICE-AGENT-109. The bytes go over HTTP to `/tool/vision-upload` (proxy of the API's `POST /uploads/vision`), and only the resulting `image_ref`, a bare filename, moves after that. The Realtime model is deliberately never shown the picture: it gets the catalogue's verdict as a tool result, which is the VOICE-AGENT-044 guardrail and must not be reopened through the image door. The voice path itself is VOICE-AGENT-180 and is not built yet; a picture question currently takes the text path and stops the audio session.
- **`image_ref` stays out of `text_tool_definitions()`.** Only the server sets it, on the forced first search of a picture turn, so a recovery re-query cannot make the API read the same photo twice (about four cents, and the question has already been composed in words). If you add a tool that should carry it, pass it server-side the way `execute_text_tool` does.
