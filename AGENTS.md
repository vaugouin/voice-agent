...
- **⚠️ Shared behavior is written TWICE — voice in JS, text in Python. Change BOTH.** The voice/Realtime path runs client-side in `app/static/app.js`; the `/text-chat` path runs server-side in `app/main.py`. Several rules therefore exist as **parallel copies in two languages**, and editing one mode silently leaves the other inconsistent (this bit us on VOICE-AGENT-112: the JS language-detection fix in 1.0.44 left the Python `/text-chat` detector wrong until 1.0.45). Known duplicated pairs to keep in sync (`app.js` ↔ `main.py`): language detection (`FRENCH_MARKERS` / `FRENCH_PHRASES` / `detectUiLanguageFromText` ↔ `detect_ui_language_from_text`), section families (`BACKGROUND_FAMILY_KEYWORDS`), background-topic words, intent stopwords (`INTENT_STOPWORDS`), and the text-normalization helpers. Before committing any change to a shared rule (especially the vocabulary lists), grep the **other** file for the same constant/function and apply the change there too. Factoring these onto a single source is tracked as VOICE-AGENT-126; until it lands, this manual check is mandatory. (The persona is already single-sourced in `SOUL.md`/`souls/` — the model to aim for.)
